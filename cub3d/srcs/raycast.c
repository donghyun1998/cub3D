...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/14 16:51:12 by donghyk2          #+#    #+#             */
/*   Updated: 2023/06/14 16:51:13 by donghyk2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_wall_element(t_cub *cub, t_ray *ray)
{
	if (ray->side == VERTICAL)
	{
		ray->wall_height = (int)(HEIGHT / ((ray->map_x - cub->player.pos.x + \
			(1 - ray->dx) / 2) / ray->dir.x));
		ray->wall_x = cub->player.pos.y + ((ray->map_x - cub->player.pos.x + \
			(1 - ray->dx) / 2) / ray->dir.x) * ray->dir.y;
	}
	else
	{
		ray->wall_height = (int)(HEIGHT / ((ray->map_y - cub->player.pos.y + \
			(1 - ray->dy) / 2) / ray->dir.y));
		ray->wall_x = cub->player.pos.x + ((ray->map_y - cub->player.pos.y + \
			(1 - ray->dy) / 2) / ray->dir.y) * ray->dir.x;
	}
